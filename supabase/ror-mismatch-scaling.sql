...
--
-- Factor: 1.0 for an even match, falling linearly to a 0.10 floor across a
-- 500-point gap.  factor = greatest(0.10, 1 - gap / 500)
--   gap 0   -> 1.00      gap 250 -> 0.50
--   gap 100 -> 0.80      gap 450+ -> 0.10
-- ============================================================================
...
begin
  ...
  mismatch := greatest(0.10, 1 - abs(c_rating - o_rating) / 500.0);

  c_new := round(c_rating + k * (c_score - c_expected) * mismatch);
  o_new := round(o_rating + k * (o_score - o_expected) * mismatch);
