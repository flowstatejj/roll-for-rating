...
begin
  ...
  return jsonb_build_array(
    ...
    jsonb_build_object('key','ksGym','period','once','title','King Slayer — beat the #1 in your gym','progress',public.king_slayer_progress(uid,'gym'),'target',1,'reward',10,'claimed', exists(select 1 from public.quest_claims where user_id=uid and quest_key='ksGym' and period_key='once')),
    jsonb_build_object('key','ksTown','period','once','title','King Slayer — beat the #1 in your town','progress',public.king_slayer_progress(uid,'city'),'target',1,'reward',12,'claimed', exists(select 1 from public.quest_claims where user_id=uid and quest_key='ksTown' and period_key='once')),
    jsonb_build_object('key','ksState','period','once','title','King Slayer — beat the #1 in your state','progress',public.king_slayer_progress(uid,'state'),'target',1,'reward',14,'claimed', exists(select 1 from public.quest_claims where user_id=uid and quest_key='ksState' and period_key='once')),
    ...
    jsonb_build_object('key','ksContinent','period','once','title','King Slayer — beat the #1 on your continent','progress',public.king_slayer_progress(uid,'continent'),'target',1,'reward',18,'claimed', exists(select 1 from public.quest_claims where user_id=uid and quest_key='ksContinent' and period_key='once')),
    jsonb_build_object('key','ksWorld','period','once','title','King Slayer — beat the world #1','progress',public.king_slayer_progress(uid,'world'),'target',1,'reward',20,'claimed', exists(select 1 from public.quest_claims where user_id=uid and quest_key='ksWorld' and period_key='once')),
