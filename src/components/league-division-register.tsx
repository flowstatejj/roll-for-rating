...
import { fetchEligibleLeagueDivisions, fetchLeagueDivisions, registerLeagueDivision } from '@/lib/leagues';
import { divisionSummary } from '@/lib/tournaments';
import { kgToLbs } from '@/lib/weight';
import type { EligibleLeagueDivision, LeagueDivision } from '@/lib/types';

/**
 * Member-facing division list for a division-based league. Same eligibility UX
 * as the tournament DivisionRegister (register by belt/age/weight/rating/gender,
 * inline capture of a missing weight/gender), but backed by the league RPCs.
 * Registering into a division also joins the league.
 */
export function LeagueDivisionRegister({ leagueId, onChanged }: { leagueId: string; onChanged?: () => void }) {
  const theme = useTheme();
  const { t } = useTranslation();

  const [divs, setDivs] = useState<LeagueDivision[]>([]);
  const [elig, setElig] = useState<EligibleLeagueDivision[]>([]);
  ...
  const reload = useCallback(async () => {
    try {
      const [d, e] = await Promise.all([fetchLeagueDivisions(leagueId), fetchEligibleLeagueDivisions(leagueId)]);
      setDivs(d);
      setElig(e);
    } catch (err) {
      console.warn('eligible league divisions load failed', err);
    } finally {
      setLoading(false);
    }
  }, [leagueId]);
  ...
  async function doRegister(ed: EligibleLeagueDivision) {
    ...
    try {
      const r = await registerLeagueDivision(ed.division_id, opts);
      if (r.ok) {
        await reload();
        onChanged?.();
      } else {
  ...
  function ineligibleReason(ed: EligibleLeagueDivision): string {
    const failed: string[] = [];
  ...
  function renderAction(ed: EligibleLeagueDivision) {
  ...
  const byId = new Map(divs.map((d): [string, LeagueDivision] => [d.id, d]));
